...
---
title: "DE Analysis (Nonlesional Skin, fig 7) II: Make Metascape Heatmaps"
...
Makes WGCNA module- and ICA factor-annotated heatmaps, derived from [Metascape](https://metascape.org) annotations of DE analysis of NL skin. 
...
final version  - includes ICA factors as well as WGCNA modules 
...
```{r to_remove, eval = FALSE, include = FALSE}
# load annotation
#ad <- get_anno_data()
# load opentargets data - not nec any more
#ot <- read_tsv("../shared_data/Open_targets/EFO_0000676-associated-diseases_not_textm_2022-11-08.tsv") 
# ot <- setNames(select(ot, 1:2), c("GeneSymbol", "OpenTargetsOverallAssociationScore"))
# ot <- read_tsv("../shared_data/Open_targets/OT_psoriasis.tsv")
```


```{r 02_4_sec, eval = FALSE, include = FALSE} 

#02_4 runs section #------
#(skipped)

# g base file path
base_file_path <- "../de_DR/02_4/"
# just for input, files downloaded from Metascape (can be same)
base_file_meta <- "../de_DR/02_4/"


# Both cohorts - Lesional - Disease Activity - df3 - All Associated ----
# Load data - 

tissue = "Lesional"; cohort = "Both"; model_type = "splinedf3"; typestr = "all"
shtiss <- short_tissues[tissue]
# grs (base_file_path <-> base_file_meta Jun 2023)
result_set <- "both_assoc_all"
file_path <- file.path(base_file_meta, "metascape/", shtiss, model_type, "/downloads/")
...
#module = "salmon"
#id <- ids[i]
#eigengene_comp(idx = i, id = id, module = module, tissue = tissue, cohort = cohort, model = model_type, drug = drug, 
#               hm_split = F, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)
#stop("1466")
## end to rm 

# Comparison heatmap of summary annotations
# copy to clipboard and show about 1150 wide x 520 high

# add_numbers = TRUE, width = 1750, overlap_norm = "none"
# comp_hm heatmap -----
factors_of_interest <- c("Factor S1", "Factor S2", "Factor S6", "Factor S9", "Factor S18", "Factor S21", "Factor S22")
filt_mdat <- comp_hm(grp = "Summary", only_sig = TRUE, add_numbers = FALSE, 
                     add_counts_to_colnames = FALSE, outd = outd, 
                     typestr = "all", modules_to_force_in = "blue",
                     width = 800, height = 340, overlap_norm = "jaccard",
                     factors_of_interest = factors_of_interest)

pway_gene_sets <- filt_mdat %>% mutate(Type = paste0(shtiss, "_", result_set)) %>% relocate(Type)
stop("1477")

## group 18 genes sect to rm -----
# get genes for group 18 (the cell cycle assoc ones) 
out_df = make_id_gene_table(18, meta_dat, meta_groups, meta_anno)
write_tsv(out_df, file = file.path(outd, "G18_term_gene_6col.txt"))
out_df2 = select(out_df, ID, symbol)
write_tsv(out_df2, file = file.path(outd, "G18_term_gene_2col.txt"))
out_df2$member = 1 
# see fish example fromm https://tidyr.tidyverse.org/reference/pivot_wider.html
out_dfw = pivot_wider(out_df2, values_from = member, names_from = ID, values_fill = 0)
node_types = bind_rows(tibble(name = out_dfw$symbol, type="Gene"),
                       tibble(name = names(out_dfw)[-1], type="ID"))
write_tsv(out_dfw, file = file.path(outd, "G18_term_gene_wide.txt"))
write_tsv(node_types, file = file.path(outd, "G18_node_table.txt"))

# end group 18 

filter_DEtable <- function(ensg, infile) {
  # relation between F and R2
  # Gordon Smyth doesn't like R2 - https://support.bioconductor.org/p/9148416/
  # est it anyway from F
  ...
  if (flip_sign)
    tt$signed_fit_range <- -1 * tt$signed_fit_range  
  tt <- mutate(tt, Rsq = F/(F + (n-p)/(p-1)))
  filter(tt, EnsemblID %in% ensg)
}

# want specifically the salmon module mitotic cell cycle genes -get out of top tables

filter_DEtable(filter(out_df, Module == "salmon" & ID == "Mitotic cell cycle") %>% pull(EnsemblID), 
               "../de_DR/02_4/Both_A_Lesional/splinedf3/A_all_genes.txt") %>% 
  write_tsv("../de_DR/02_4/comparisons_LS/Both/splinedf3/A_salmon_mitotic.tsv")
filter_DEtable(filter(out_df, Module == "salmon" & ID == "Mitotic cell cycle") %>% pull(EnsemblID), 
               "../de_DR/02_4/Both_U_Lesional/splinedf3/U_all_genes.txt") %>% 
  write_tsv("../de_DR/02_4/comparisons_LS/Both/splinedf3/U_salmon_mitotic.tsv")

# opentargets data loaded above 

## end filter de table section ----
stop("1318")
# 
# select metascape pathways 
ids <- meta_groups$ID[which(meta_groups$Type == "Summary")]
# [2] "M5885 NABA MATRISOME ASSOCIATED"
# [12] "M5884 NABA CORE MATRISOME"   

# how diff are the genes in each metascape set? - make a quick upset plot. All genes, not module-overlap
# should I put this within comp_hm to show that somehow?

make_upsetplot <- function (nsets = 12) {
  usd <- list()
  for(id in ids[1:nsets]){
    id_genes <- strsplit(meta_dat$GeneID[which(meta_dat$ID == id)], split = "|", fixed = T)[[1]]
    usd[[id]] <- id_genes
  }
  upset(fromList(usd),  nsets = nsets, nintersects = 25)
}
...
# LS Dot plots -----
# rougly w=1000 h=800 for ~ 70 gene modules 

keydf = NULL
gene_dot_data = list()
outd = file.path(base_file_path, paste0("comparisons_", shtiss), 
                 cohort, model_type, "gene_dotplots") 
if (! dir.exists(outd))
  dir.create(outd, recursive = TRUE)
# Makes most sense to put in order, as files will appear in that order in file viewer anyway 


# further comp_hm for pway in  18 (cell cycle etc)
filt_mdat_i <- comp_hm(grp = "G18", only_sig = TRUE, add_numbers = TRUE, 
                       add_counts_to_colnames = FALSE, 
                     outd = outd, typestr = "all", 
                     width = 850, height = 340, overlap_norm = "jaccard")
filt_mdat_i <- comp_hm(grp = "G18", only_sig = TRUE, add_numbers = TRUE, 
                       add_counts_to_colnames = FALSE, 
                       outd = outd, typestr = "all", 
...
for (idx in c(1, 2, 3, 4, 10, 18)) {
  this_id <- ids[idx]
  cat("doing", this_id, " -- index", idx, "\n")
  # outcd is a list of df and gene_data
  out_cd <- comp_dot(tissue, cohort, model_type, typestr, outd, ids, idx)
  keydf <- bind_rows(keydf, out_cd$df)
  gene_dot_data[[this_id]] <- out_cd$gene_data$Gene
}
keyfile <- file.path(outd, paste0("key_dot_", typestr, ".csv"))
write_tsv(keydf, file = keyfile)
upset(fromList(gene_dot_data),  nintersects = 20, sets = rev(names(gene_dot_data)), keep.order = TRUE, order.by = "freq")
svg(filename = file.path(outd, "upset_plot.svg"), w = 7, h = 4)
upset(fromList(gene_dot_data),  nintersects = 20, sets = rev(names(gene_dot_data)), keep.order = TRUE, order.by = "freq")
dev.off()

# stop("normal (temporary) stop")
...
for (drug in c("A", "U")) {
  ...
  for (i in c(2,12)) {
    id <- ids[i]
    cat("Metascape pway", id, "\n")
    ## module <- "turquoise" # "salmon"
    for (module in c( "turquoise", "yellow")) {
      df = gene_hm(idx = i, id = id, module = module, tissue = tissue, cohort = cohort, model = model_type, drug = drug, 
                   hm_split = F, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)
      if (interact)
        xx <- readline("pnctc")
      keydf = bind_rows(keydf, df)
    }
  ...
  keyfile = file.path(base_file_path, paste(cohort, drug, tissue, sep = "_"), 
                        model_type, "gene_heatmaps", paste0("key_", typestr, ".csv"))
  write_tsv(keydf, file = keyfile)
}
stop("temp stop")

# Both cohorts - Lesional - Disease Activity - df3 - Unique ----

cat("took out the drug_unique section")
...
pway_gene_sets <- bind_rows(pway_gene_sets, 
                            filt_mdat %>% mutate(Type = paste0(shtiss, "_", result_set)) %>% relocate(Type))
# select metascape pathways 
ids <- meta_groups$ID[which(meta_groups$Type == "Summary")]
...
# heatmaps - might be better not to split by module here, as sometimes have only one or two 
# did try c(1,2,4,7,9)
for (i in c(1,2,3,6)) {
  id <- ids[i]
  cat("Metascape pway", id, "\n")
  for (module in c("turquoise", "blue")) {
    for (drug in c("A", "U")) {
      gene_hm(idx = i, id = id, module = module, tissue = tissue, cohort = cohort, model = model_type, drug = drug, 
            hm_split = F, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)
...
gene_hm(idx = 2, id = ids[2], module = "turquoise", tissue = tissue, cohort = cohort, model = model_type, drug = "A", 
        hm_split = F, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)

# all the others
idxblk = c(3,5,6,8,10:20) # or (2,3,4), (9,12,15). No overlap with U here, so don't do U
id = ids[idxblk]
cat("Metascape pway", id, "\n")
for (module in c("turquoise", "blue")) { 
  gene_hm(idx = idxblk, id = id, module = module, tissue = tissue, cohort = cohort, model = model_type, drug = "A", 
          hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)
  
}


# Both cohorts - Blood - Disease Activity - df3 - All Associated ----


tissue = "Blood"; cohort = "Both"; model_type = "splinedf3"; typestr = "all"
shtiss <- short_tissues[tissue]
...
# want a column of zeros for the Ustekinumab for blood, where there was no data from Metascape(no DEGs to give it) 
if (ncol(meta_dat) == 3) {
  meta_dat$U_assoc_all_BL_splinedf3 <- 0
  meta_dat <- relocate(meta_dat, U_assoc_all_BL_splinedf3, .after = A_assoc_all_BL_splinedf3) 
}
# Comparison heatmap of summary annotations
# copy to clipboard and show about 1300 wide x 500 high
outd <- file.path(base_file_path, paste0("comparisons_", shtiss), cohort, model_type)
factors_of_interest = c("Factor B1")
filt_mdat <- comp_hm(grp = "Summary", only_sig = TRUE, add_numbers = FALSE, 
                     typestr = "all", add_counts_to_colnames = FALSE, outd = outd, 
                     modules_to_force_in = "plum", 
                     width = 700, height = 340, overlap_norm = "jaccard",
                     factors_of_interest = factors_of_interest)
...
# how diff are the genes in each metascape set? - make a quick upset plot. All genes, not module-overlap
# should I put this within comp_hm to show that somehow?
# make_upsetplot(nsets = 10)


# dot plots. idx 1, Neutrophil Degranulation, is most important 
# 7 is Respiratory electron transport
# g was checking the degranulation things here 
...
if (! dir.exists(outd))
  dir.create(outd, recursive = TRUE)
# Makes most sense to put in order, as files will appear in that order in file viewer anyway 
# took out loop over other idx 
for (idx in c(6,1)) {
  this_id <- ids[idx]
  cat("doing", this_id, " -- index", idx, "\n")
# outcd is a list of df and gene_data
  out_cd <- comp_dot(tissue, cohort, model_type, typestr, outd, ids, idx, fig_height = 3.5, rad_scale = 1.75)
  keydf <- bind_rows(keydf, out_cd$df)
}
...
Main Program: run for Nonlesional skin

```{r 02_5_runs, fig.width=9.5, fig.height=5.5}
...
```{r 02_5_runs_ctd, eval = FALSE, include=FALSE}

### 02_5_ctd

calls to make similar heatmap for Blood and Lesional (not run)
...
# Rest of script  -- mostly df2 and pasi0 stuff - not recently updated ----to delete?  ; blood heatmaps ------ 
# 1/2 tq w=885 h=532   nothing in yellow or U
for (i in c(1,2,8)) {
  id <- ids[i]
  cat("Metascape pway", id, "\n")
  for (module in c("turquoise")) {
    for (drug in c("A", "U")) {
      gene_hm(idx = i, id = id, module = module, tissue = tissue, cohort = cohort, model = model_type, drug = drug, 
              hm_split = F, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)
      if (interact)
        xx <- readline("press any key")
    }
  }
}


stop("26 jan 23 - not revised stuff below - splinedf2 and pasizero ")
stop("temp stop")
# Both cohorts - Lesional - Disease Activity - df2 - All Associated ----
if(TRUE) {
  # Load data - grs 
  result_set <- "both_assoc_coex"
  file_path <- file.path(base_file_path, "metascape/LS/splinedf2/downloads/")
  dat <- load_data(result_set = result_set, file_path = file_path)
  meta_anno <- dat$meta_anno
  meta_groups <- dat$meta_groups
  meta_dat <- dat$meta_dat
  ...
  # Comparison heatmap of summary annotations
  # copy to clipboard and show about 1300 wide x 500 high
  comp_hm(grp = "Summary", only_sig = TRUE)
  # don't add to pway gene sets
  # select metascape pathways 
  ids <- meta_groups$ID[which(meta_groups$Type == "Summary")]
  # [2] "M5885 NABA MATRISOME ASSOCIATED"
  # [12] "M5884 NABA CORE MATRISOME"   
  ...
  for (i in c(1,3,4)) {
    ...
    ## module <- "turquoise" # "salmon"
    for (module in c("turquoise", "yellow")) {
      gene_hm(idx = i, id = id, module = module, tissue = "Lesional", cohort = "Both", model = "splinedf3", drug = "A", 
              hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = "all")
      gene_hm(idx = i, id = id, module = module, tissue = "Lesional", cohort = "Both", model = "splinedf3", drug = "U", 
              hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = "all")
    }
  }
  i=16
  ...
  cat("Metascape pway", id, "\n")
  ## 
  for (module in c("turquoise", "salmon")) {
    gene_hm(idx = i, id = id, module = module, tissue = "Lesional", cohort = "Both", model = "splinedf3", drug = "A", 
            hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = "all")
    gene_hm(idx = i, id = id, module = module, tissue = "Lesional", cohort = "Both", model = "splinedf3", drug = "U", 
            hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = "all")
...
# Both cohorts - Nonlesional - Disease Activity - df3 - Unique ----

typestr = "unique"

# Load data - grs 
result_set <- "both_unique_coex"
file_path <- file.path(base_file_path, paste0("metascape/",shtiss,"/splinedf3/downloads/"))
dat <- load_data(result_set = result_set, file_path = file_path)
meta_anno <- dat$meta_anno
meta_groups <- dat$meta_groups
meta_dat <- dat$meta_dat
rm(dat)
# Comparison heatmap of summary annotations
# copy to clipboard and show about 850 wide x 520 high

outd <- file.path(base_file_path, paste0("comparisons_", shtiss), cohort, model_type)
filt_mdat <- comp_hm(grp = "Summary", only_sig = TRUE, add_numbers = FALSE, 
                     add_counts_to_colnames = FALSE, 
                     outd = outd, typestr,  width = 600, height = 340, overlap_norm = "jaccard")
...
# heatmaps
# dont bother adding to gene_sets as are not very interesting 
ids <- meta_groups$ID[which(meta_groups$Type == "Summary")]
for (i in c(1,3,4)) {
  id <- ids[i]
  cat("Metascape pway", id, "\n")
  for (module in c("turquoise")) {
    for (drug in c("A", "U"))
      gene_hm(idx = i, id = id, module = module, tissue = tissue, cohort = cohort, model = model_type, drug = drug, 
            hm_split = FALSE, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)
  }
}

#idxblk = c(1:3,5:8,10:14,16:20) # or (2,3,4), (9,12,15). No overlap with U here, so don't do U
#id = ids[idxblk]
#cat("Metascape pway", id, "\n")
# module = "turquoise" ; drug = "A"
#gene_hm(idx = i, id = id, module = module, tissue = tissue, cohort = cohort, model = model_type, drug = drug, 
#        hm_split = FALSE, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = typestr)
#
#  
#
  ...
  # Both cohorts - Lesional - Disease Activity - df2 - Unique ----
  
  # Load data - grs 
  result_set = "both_unique_coex"
  file_path <- file.path(base_file_path, "metascape/NL/splinedf2/downloads/")
  dat <- load_data(result_set = result_set, file_path = file_path)
  meta_anno <- dat$meta_anno
  meta_groups <- dat$meta_groups
  meta_dat <- dat$meta_dat
  ...
  # Comparison heatmap of summary annotations
  # copy to clipboard and show about 1300 wide x 500 high
  comp_hm(grp = "Summary", only_sig = TRUE, add_numbers = FALSE)
  # dont add to gene sets 
  ids <- meta_groups$ID[which(meta_groups$Type == "Summary")]
  # 
  
  # [2] "GO:0001501 skeletal system development" 
  # might want to put sthg else in the heatmap file (the pway id string, but is long? or the index , 2 here?)
  
  # these have small p-vals 
  for (i in c(1,7,8,18)) {
    id <- ids[i]
    cat("Metascape pway", id, "\n")
    for (module in c("turquoise", "yellow")) { 
      gene_hm(idx = i, id = id, module = module, tissue = "Lesional", cohort = "Both", model = "splinedf3", drug = "A", 
              hm_split = T, row_font_size = 10, clin_anno = F, typestr = "unique")
      gene_hm(idx = i, id = id, module = module, tissue = "Lesional", cohort = "Both", model = "splinedf3", drug = "U", 
              hm_split = T, row_font_size = 10, clin_anno = F, typestr = "unique")
    }
  }
  
  # these collectively show as zero in 0
  idxblk = c(2,3,4, 9,12,15) # or (2,3,4), (9,12,15). No overlap with U here, so don't do U
  id = ids[idxblk]
  cat("Metascape pway", id, "\n")
  for (module in c("turquoise", "yellow")) { 
    gene_hm(idx = idxblk, id = id, module = module, tissue = "Lesional", cohort = "Both", model = "splinedf3", drug = "A", 
            hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = "unique")
...
# 4. PASIZERO gene sets 

# Both cohorts - PASI(0) - Disease Activity - df2 - All Associated ----

# Load data - grs 
result_set <- "both_assoc_coex"
file_path <- file.path(base_file_path, "metascape/pasizero/splinedf2/downloads/")
dat <- load_data(result_set = result_set, file_path = file_path)
meta_anno <- dat$meta_anno
meta_groups <- dat$meta_groups
meta_dat <- dat$meta_dat
...
# Comparison heatmap of summary annotations
# copy to clipboard and show about 1300 wide x 500 high
filt_mdat <- comp_hm(grp = "Summary", only_sig = TRUE)
pway_gene_sets <- bind_rows(pway_gene_sets, 
                            filt_mdat %>% mutate(Type = paste0("pasizero_", result_set)) %>% relocate(Type))
# select metascape pathways  w=972, h=532
ids <- meta_groups$ID[which(meta_groups$Type == "Summary")]

# g was checking the degranulation things here 
# 
# g filter(meta_dat, grepl("degranulation", ID))
# g filter(meta_groups, grepl("degranulation", ID))
# g filter(meta_groups, "G1"== Group)
...
for (i in c(1,2,3)) {
  ...
  for (module in c("turquoise")) {
    for (drug in c("A", "U")) {
      gene_hm(idx = i, id = id, module = module, tissue = tissue, cohort = "Both", model = "splinedf2", drug = drug, 
              hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F, typestr = "all")
      if (interact)
        xx <- readline("press any key")
    }
...
# output pway gene sets 
file_outd <- file.path(base_file_path, "metascape")
outf <- file.path(file_outd, "metascape_pathway_gene_sets.txt")
cat("making", outf, "\n")
write_tsv(pway_gene_sets, file = outf)

stop()
# Rest of script - Ashley's stuff below ----
# what is gene_score for




gene_score(id = id, module = module, de = "Lesional_Both_splinedf3")

gene_score(id = id, module = module, de = "LS_UST_wk01_vs_wk00")
gene_hm(id = id, module = module, de = "LS_ADA_wk01_vs_wk00", hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F)
gene_score(id = id, module = module, de = "LS_ADA_wk01_vs_wk00")
id <- ids[15]
module <- "tan"
gene_hm(id = id, module = module, de = "LS_UST_wk01_vs_wk00", hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F)
gene_score(id = id, module = module, de = "LS_UST_wk01_vs_wk00")
gene_hm(id = id, module = module, de = "LS_ADA_wk01_vs_wk00", hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F)
gene_score(id = id, module = module, de = "LS_ADA_wk01_vs_wk00")

# Both cohorts - Lesional - Treatment ####

# Load data
result_set = "Both_LS_treat_sig"
dat <- load_data(result_set = result_set)
meta_anno <- dat$meta_anno
meta_groups <- dat$meta_groups
meta_dat <- dat$meta_dat
rm(dat)
...
# Comparison heatmap of summary annotations
comp_hm(grp = "Summary", only_sig = T)
...
# Both cohorts - Lesional - Response ####

# Load data
result_set = "Both_LS_resp_sig"
dat <- load_data(result_set = result_set)
meta_anno <- dat$meta_anno
meta_groups <- dat$meta_groups
meta_dat <- dat$meta_dat
...
id <- ids[1]
module <- "yellow"
gene_hm(id = id, module = module, de = "LS_UST_wk01_Resp", hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F)
gene_score(id = id, module = module, de = "LS_UST_wk01_Resp")
gene_hm(id = id, module = module, de = "LS_ADA_wk01_Resp", hm_split = T, row_font_size = NULL, row_annot_hm = F, clin_anno = F)
gene_score(id = id, module = module, de = "LS_ADA_wk01_Resp")
